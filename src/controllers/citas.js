...
const Citas 	= require('../collections/citas');
...
function saveCita(req, res, next){
	console.log(JSON.stringify(req.body));

	Cita.forge({ id_orden_servicio:req.body.id_orden_servicio ,id_tipo_cita:req.body.id_tipo_cita ,id_bloque_horario:req.body.id_bloque_horario  })
	.save()
	.then(function(data){
		res.status(200).json({
			error: false,
...
function getCitaById(req, res, next) {
...
function updateCita(req, res, next) {
	const id = Number.parseInt(req.params.id);
	if (!id || id == 'NaN') {
		return res.status(400).json({ 
			error: true, 
...
function deleteCita(req, res, next) {
	const id = Number.parseInt(req.params.id);
	if (!id || id == 'NaN') {
		return res.status(400).json({ 
			error: true, 
